...
const outputDir = path.resolve('artifacts/mobile-v3');
...
if (!openRound) throw new Error('No open round is available for Mobile V3 testing');
...
for (const viewport of viewports) {
  ...
  try {
    ...
    const dayUrl = `${baseUrl}/m3/day/${reviewRound.business_date}/${reviewRound.source_id}?bucket=review`;
    ...
    if (await firstPair.count()) {
      ...
      await page.getByRole('button', { name: /ข้อความ LINE/ }).click();
      await page.locator('.chat-sheet').waitFor({ state: 'visible' });
      await page.locator('.chat-sheet .icon-button').click();
      await page.getByRole('button', { name: /AI เลือกคู่/ }).click();
      await page.locator('.ai-reasons').waitFor({ state: 'visible' });
      await page.locator('.chat-sheet .icon-button').click();
      await page.getByRole('button', { name: 'เลือกเอกสารคู่ใหม่' }).click();
    ...
    await page.screenshot({ path: path.join(outputDir, `mobile-v3-${viewport.width}.png`), fullPage: false });
  } catch (error) {
