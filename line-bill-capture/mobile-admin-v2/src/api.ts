...
const waitShadowRun = async (runId: string): Promise<Row> => {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    const run: Row = await rawRequest<Row>(`/api/admin/agents/runs/${runId}`).catch((error) => ({ status: 'failed', error_message: error.message }));
    if (['completed', 'failed', 'skipped'].includes(String(run?.status || ''))) return run;
    await new Promise((resolve) => setTimeout(resolve, 500));
  }
  return { status: 'failed', error_message: 'AI ใช้เวลานานเกินไป กรุณาให้เหตุผลจากหลักฐานที่ตรวจเอง' } as Row;
};
...
const askReason = (actionKey: string, body: Record<string, any> = {}, evidence: Row[] = [], shadowRunId = '') => new Promise<{ reason_code: string; reason_text: string; evidence_message_ids: number[] }>((resolve, reject) => {
  ...
  void waitShadowRun(shadowRunId).then((run) => {
    if (!root.isConnected) return;
    if (run.status === 'completed') {
      const supported = run.predicted_action === actionKey; const confidence = Math.round(Number(run.confidence || 0) * 100); const risks = Array.isArray(run.risk_flags) ? run.risk_flags : [];
      shadow.className = `decision-sheet-shadow ${supported && !risks.length ? '' : 'hold'}`;
      shadow.innerHTML = `<strong>${supported ? 'AI เห็นว่าทำรายการนี้ต่อได้' : 'AI แนะนำให้พักตรวจ'} · มั่นใจ ${confidence}%</strong><p>${escapeHtml(run.rationale || 'AI ไม่ได้ระบุเหตุผล')}</p>${risks.length ? `<small>จุดเสี่ยง: ${escapeHtml(risks.join(' · '))}</small>` : ''}<button type="button">ใช้เหตุผล AI เป็นร่าง</button>`;
      (shadow.querySelector('button') as HTMLButtonElement).onclick = () => choose('other', String(run.rationale || ''));
    } else { shadow.className = 'decision-sheet-shadow failed'; shadow.innerHTML = `<strong>AI วิเคราะห์รอบนี้ไม่ได้</strong><p>${run.status === 'skipped' ? 'ยังไม่ได้เปิด Shadow AI บนระบบนี้' : escapeHtml(run.error_message || 'ให้ใช้หลักฐานที่ตรวจเอง')}</p>`; }
  });
});
