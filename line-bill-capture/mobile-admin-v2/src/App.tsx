...
const bucketInfo: Record<Bucket, { label: string; tone: string }> = {
  ai_pending: { label: 'AI กำลังอ่าน', tone: 'blue' },
  ...
  support: { label: 'เอกสารประกอบ', tone: 'blue' },
...
function groupName(sourceId: string, groups: Row[] = []) {
  ...
  const configured = String(group?.display_name || '').trim();
  return GROUP_NAMES[sourceId] || (configured && configured !== sourceId ? configured : `กลุ่ม ${sourceId.slice(-5)}`);
}
...
function ErrorBox({ error }: { error: unknown }) {
  return error ? <div className="notice error"><AlertTriangle size={18} /><span>{String((error as Error).message || error)}</span><button onClick={() => window.location.reload()}>ลองใหม่</button></div> : null;
}
...
function WorkHome() {
  ...
  const rows = useMemo(() => (days.data || []).filter((row) => !sourceFilter || row.source_id === sourceFilter), [days.data, sourceFilter]);
  const urgent = useMemo(() => rows.filter((row) => urgency(row) > 0 && row.closing_status !== 'closed')
    .sort((a, b) => String(b.business_date).localeCompare(String(a.business_date)) || urgency(b) - urgency(a)), [rows]);
  ...
  const next = selectNextRound(urgent, today);
  const nextBucket: Bucket = Number(next?.needs_amount_count || 0)
    ? 'needs_amount'
    : Number(next?.pending_count || 0)
      ? 'review'
      : Number(next?.processing_count || 0)
        ? 'ai_pending'
        : 'bill';
  ...
  return <main>
    <PageHeader title="งานเอกสาร" subtitle="เปิดแล้วทำรายการสำคัญที่สุดก่อน" />
    <ErrorBox error={days.error || groups.error} />
    <div className="group-filter" role="group" aria-label="กรองกลุ่ม LINE">
      <button className={!sourceFilter ? 'active' : ''} onClick={() => setSourceFilter('')}>ทุกกลุ่ม</button>
      {(groups.data || []).map((row) => <button key={row.source_id} className={sourceFilter === row.source_id ? 'active' : ''} onClick={() => setSourceFilter(row.source_id)}>{groupName(row.source_id, groups.data)}</button>)}
    </div>
    {next && <section className="focus-card">
      <div className="focus-label"><span>{next.business_date === today ? 'งานวันนี้' : 'งานย้อนหลังล่าสุด'}</span><b>ทำต่อเนื่องในรอบเดียว</b></div>
      ...
      <Link className="primary-button" to={`/day/${next.business_date}/${next.source_id}?bucket=${nextBucket}`}><Eye /> เริ่มตรวจ</Link>
    </section>}
    {!next && !days.isLoading && <section className="all-clear"><BadgeCheck /><div><strong>ไม่มีงานค้างในตัวกรองนี้</strong><span>เลือกกลุ่มอื่นหรือเปิดปฏิทินเพื่อดูรอบเก่า</span></div></section>}
    ...
    <section className="section-block">
      <div className="section-title"><h2>งานย้อนหลัง</h2><Link to="/calendar">ดูปฏิทิน</Link></div>
...
function buildQueueEntries(dayRows: Row[], poolRows: Row[], matches: Row[]) {
  ...
  dayRows.forEach((item) => {
    ...
    // Transfer-paid transactions are listed on the slip date by the matches API.
    // A confirmed bill on its older document date must not become a duplicate done row.
  ...
  const priority: Record<Bucket, number> = { needs_amount: 0, review: 1, bill: 2, slip: 2, ai_pending: 3, support: 4, other: 5, done: 6 };
...
function DayPage() {
  ...
  const todo = entries.filter((entry) => ['review', 'needs_amount', 'bill', 'slip', 'ai_pending'].includes(entry.bucket));
  const done = entries.filter((entry) => entry.bucket === 'done'); const other = entries.filter((entry) => ['support', 'other'].includes(entry.bucket));
...
function ReviewPage() {
  ...
  const [note, setNote] = useState(''); const [picker, setPicker] = useState(false);
  const [receipt, setReceipt] = useState(false); const [batch, setBatch] = useState(false);
  const [cashForm, setCashForm] = useState(false);
  const [preview, setPreview] = useState<Row | null>(null); const [contextMode, setContextMode] = useState<'chat' | 'ai' | null>(null);
  const [activeEvidenceId, setActiveEvidenceId] = useState(selectedItemId);
  const [amountEditor, setAmountEditor] = useState(false); const [classifyReason, setClassifyReason] = useState(false);
  const [reclassify, setReclassify] = useState<string | null>(null);
  const [amountNotice, setAmountNotice] = useState('');
  useEffect(() => { setActiveEvidenceId(Number(bill?.id || slip?.id || item?.id || selectedItemId)); }, [bill?.id, slip?.id, item?.id, selectedItemId]);
  const contextItemId = Number((item || bill || slip)?.id || 0);
  const context = useQuery({ queryKey: ['context', contextItemId], enabled: Boolean(contextItemId && contextMode === 'chat'), queryFn: () => api.context(contextItemId) });
  ...
  // ถามเหตุผลเฉพาะตอนที่ AI เคยมั่นใจแล้วผิด — เคสนั้นคือ AI เข้าใจอะไรผิดจริงจัง คุ้มที่จะพิมพ์อธิบาย
  ...
  const classify = async (category: string, reason = '', learningResponse = '') => {
    ...
    const result = await api.mutate(`/api/admin/items/${item?.id}/category`, {
      ...
      ai_learning_approved: Boolean(learningResponse),
      learning_response: learningResponse,
      // มีเหตุผลแต่ไม่มีคำตอบจาก AI = ผู้ใช้เลือกไม่สอน จึงไม่ต้องเก็บเป็นตัวอย่าง
      record_learning: !(reason && !learningResponse)
    }, 'PUT');
  ...
  return <main className="review-page">
    ...
    {amountNotice && <div className="notice"><Bot size={18} /><span>{amountNotice}</span></div>}
    {bill && slip && <div className="evidence-switch" role="tablist" aria-label="เลือกหลักฐาน">
      <button className={activeEvidence?.id === bill.id ? 'active' : ''} onClick={() => setActiveEvidenceId(Number(bill.id))}><span>บิล</span><strong>{money(itemAmount(bill))}</strong></button>
      <button className={activeEvidence?.id === slip.id ? 'active' : ''} onClick={() => setActiveEvidenceId(Number(slip.id))}><span>สลิป</span><strong>{money(itemAmount(slip))}</strong></button>
    </div>}
    {activeEvidence && !isCash && <EvidenceStage label={activeEvidence.id === bill?.id ? 'บิล' : activeEvidence.id === slip?.id ? 'สลิป' : 'เอกสาร'} item={activeEvidence} onOpen={() => setPreview(activeEvidence)} />}
    {isCash && item && <CashEvidenceCard item={item} />}
    <div className="context-actions">
      <button className="context-button" onClick={() => setContextMode('chat')}><MessageCircle /><span><strong>แชทรอบรูป</strong><small>ผู้ส่งและข้อความใกล้เคียง</small></span><ChevronRight /></button>
      {match && <button className="context-button" onClick={() => setContextMode('ai')}><Bot /><span><strong>เหตุผลของ AI</strong><small>ตรวจว่าระบบใช้ข้อมูลอะไรจับคู่</small></span><ChevronRight /></button>}
    </div>
    ...
    <div className={`sticky-actions ${match ? 'decision-actions' : ''}`}>
      {match && !isConfirmed && <>
        <div className={`sticky-compare ${exact ? 'exact' : 'mismatch'}`}><span>บิล <b>{money(itemAmount(bill || {}))}</b></span><span>สลิป <b>{money(itemAmount(slip || {}))}</b></span><span>ต่าง <b>{money(Math.abs(difference))}</b></span></div>
        {!exact && <button className="secondary-button edit-amount-action" onClick={() => setAmountEditor(true)}>แก้ยอดบิล</button>}
        <div className="action-row"><button className="secondary-button" onClick={() => setPicker(true)}>เปลี่ยนคู่</button>
        <button className="danger-button" onClick={() => mutateMatch.mutate('rejected')}>ไม่ใช่คู่</button>
        <button className="primary-button grow" onClick={() => mutateMatch.mutate('confirmed')} disabled={!exact || mutateMatch.isPending}><Check /> {nextEntry ? 'ยืนยันและถัดไป' : 'ยืนยัน'}</button></div>
      </>}
    ...
    {contextMode && <div className="sheet-backdrop"><section className="full-sheet chat-sheet"><header><div><h2>{contextMode === 'chat' ? 'แชทรอบเอกสาร' : 'เหตุผลของ AI'}</h2><p>{groupName(sourceId)} · {shortDate(date)}</p></div><button className="icon-button" onClick={() => setContextMode(null)}><X /></button></header>{contextMode === 'chat' ? <ChatContext rows={context.data?.messages || []} loading={context.isLoading} /> : <AiReasons match={match} bill={bill} slip={slip} />}</section></div>}
    {amountEditor && (match ? bill : item) && <AmountEditorSheet item={(match ? bill : item)!} onClose={() => setAmountEditor(false)} onSaved={amountSaved} />}
    {classifyReason && item && <ReasonSheet item={item} targetCategory="other" prompt="รูปนี้คืออะไร" title={`ระบุเหตุผลที่ไม่ใช่${item.category?.startsWith('bill') ? 'บิล' : 'สลิป'}`} onClose={() => setClassifyReason(false)} onSave={(reason, response) => classify('other', reason, response)} />}
    {reclassify && item && <ReasonSheet item={item} targetCategory={reclassify} prompt={`ทำไมรูปนี้ถึงเป็น${CATEGORY_TH[reclassify] || reclassify}`} title={`AI เคยมั่นใจว่าไม่ใช่${CATEGORY_TH[reclassify] || reclassify}`} onClose={() => setReclassify(null)} onSave={(reason, response) => classify(reclassify, reason, response)} />}
...
function EvidenceStage({ label, item, onOpen }: { label: string; item: Row; onOpen: () => void }) {
  return <button className="evidence-stage" onClick={onOpen}><span className="evidence-label">{label}</span><img src={imageUrl(item.id)} alt={label} /><div><span><strong>{money(itemAmount(item))}</strong><small>บาท</small></span><p>{item.vendor_name || item.supplier_name || item.ai_title || `รูป #${item.id}`}</p><b><ZoomIn /> แตะเพื่อซูม</b></div></button>;
}

function AiReasons({ match, bill, slip }: { match?: Row | null; bill?: Row | null; slip?: Row | null }) {
  let reasons: string[] = [];
  try { reasons = Array.isArray(match?.reasons) ? match.reasons : JSON.parse(match?.reason_json || '[]'); } catch { reasons = []; }
  return <section className="ai-reasons"><div className="ai-score"><Bot /><span><small>คะแนนที่เสนอ</small><strong>{Number(match?.score || 0)}%</strong></span></div><ol>{reasons.filter(Boolean).map((reason, index) => <li key={`${reason}-${index}`}>{reason}</li>)}</ol>{!reasons.length && <Empty text="AI ไม่ได้บันทึกเหตุผลสำหรับคู่นี้" />}<div className="ai-evidence"><span>บิล: {bill?.ai_summary || 'ไม่มีสรุป'}</span><span>สลิป: {slip?.ai_summary || 'ไม่มีสรุป'}</span></div></section>;
}
...
function OperationalTools({ item, onReceipt, onBatch }: { item: Row; onReceipt: () => void; onBatch: () => void }) {
  ...
  return <section className="operation-tools">
    ...
    {showTransfer && <TransferRequestSheet item={item} onClose={() => setShowTransfer(false)} onSent={async () => { setShowTransfer(false); await qc.invalidateQueries(); }} />}
  </section>;
}

function TransferRequestSheet({ item, onClose, onSent }: { item: Row; onClose: () => void; onSent: () => void | Promise<void> }) {
  const targetGroup = GROUP_NAMES[item.source_id] || item.source_name || `กลุ่ม ${String(item.source_id || '').slice(-5)}`;
  const [text, setText] = useState(`แจ้งให้โอน\nรายการ: ${item.vendor_name || item.supplier_name || item.bill_purpose || `บิลรูป #${item.id}`}\nยอด: ${money(itemAmount(item))} บาท\nวันที่บิล: ${shortDate(businessDate(item))}`);
  const [checked, setChecked] = useState(false);
  const mutation = useMutation({
    mutationFn: () => api.mutate(`/api/admin/items/${item.id}/request-transfer`, {
      text,
      preview_item_id: Number(item.id),
...
function AmountEditorSheet({ item, onClose, onSaved }: { item: Row; onClose: () => void; onSaved: (result: Row) => void | Promise<void> }) {
  const [amount, setAmount] = useState(String(itemAmount(item) || ''));
  const isSlip = ['transfer', 'transfer_notice', 'incoming_transfer'].includes(item.category);
  const label = isSlip ? 'สลิป' : 'บิล';
  const mutation = useMutation({
    mutationFn: () => api.mutate(`/api/admin/items/${item.id}`, isSlip ? { slip_amount_text: amount } : { bill_total_text: amount }, 'PATCH'),
    onSuccess: (result) => onSaved(result)
  });
  const parsed = Number(String(amount).replace(/,/g, ''));
  return <div className="sheet-backdrop"><section className="choice-sheet form-sheet"><header><div><h2>แก้ยอด{label}</h2><p>ใช้เมื่อ AI อ่านตัวเลขบน{label}ผิด ระบบจะจำค่าที่แก้ด้วยมือไว้</p></div><button className="icon-button" onClick={onClose}><X /></button></header>
...
const decisionWasCancelled = (error: unknown) => (error as { code?: string })?.code === 'decision_cancelled';
...
function ReasonSheet({ item, title, targetCategory, prompt, onClose, onSave }: { item: Row; title: string; targetCategory: string; prompt: string; onClose: () => void; onSave: (reason: string, response: string) => void | Promise<void> }) {
  ...
  const save = async () => {
    ...
    try {
      // ไม่ติ๊กสอน AI = แก้ประเภทอย่างเดียว ไม่ต้องผ่านขั้นให้ AI ทวนความเข้าใจ
      // และที่สำคัญคือ AI จะขวางการแก้ประเภทไม่ได้
      if (!learn) { await onSave(teaching, ''); return; }
      const review = await api.mutate(`/api/admin/items/${item.id}/category-learning/review`, { reason: teaching, target_category: targetCategory });
      if (review.decision === 'clarify') { setQuestion(review.question || 'ช่วยระบุเพิ่มว่ารูปนี้คืออะไร'); return; }
  ...
  return <div className="sheet-backdrop"><section className="choice-sheet form-sheet"><header><div><h2>{title}</h2><p>AI จะดูรูปและเหตุผลก่อนเก็บเป็นตัวอย่างสำหรับครั้งถัดไป</p></div><button className="icon-button" onClick={onClose}><X /></button></header>
...
function CandidatePicker({ current, bill, slip, date, sourceId, onClose }: { current: Row; bill?: Row | null; slip?: Row | null; date: string; sourceId: string; onClose: () => void }) {
  ...
  const [days, setDays] = useState(7); const [allGroups, setAllGroups] = useState(false); const [query, setQuery] = useState(''); const [preview, setPreview] = useState<Row | null>(null);
  ...
  return <div className="sheet-backdrop"><section className="full-sheet">
    ...
    <div className="candidate-list">{rows.map((row) => { const diff = Math.abs(itemAmount(row) - baseAmount); return <article className="candidate-card" key={row.id}>
      ...
      <button className="candidate-select" type="button" onClick={() => choose(row)}><Check /> เลือก{needSlip ? 'สลิป' : 'บิล'}นี้</button>
    </article>; })}</div>
...
function GroupBuilder() {
  ...
  const { pool } = useDayData(date, sourceId); const [bills, setBills] = useState<number[]>([]); const [slips, setSlips] = useState<number[]>([]); const [sortMode, setSortMode] = useState<GroupDocumentSort>('ai');
  ...
  if (pool.isLoading) return <main><PageHeader back title="รวมบิลและสลิป" subtitle={`${groupName(sourceId)} · ${shortDate(date)}`} /><Loading /></main>;
  return <main className="group-page"><PageHeader back title="รวมบิลและสลิป" subtitle={`${groupName(sourceId)} · รอบวันที่ ${shortDate(date)} · ค้นหา ±14 วัน`} />
    ...
    <ErrorBox error={mutation.error} /><div className="group-submit"><button className="primary-button" disabled={!bills.length || !slips.length || Math.abs(totals.difference) >= .01 || mutation.isPending} onClick={() => window.confirm('ยอดรวมตรงกันแล้ว ยืนยันสร้างชุดนี้เพื่อรอตรวจภาพทุกใบใช่หรือไม่') && mutation.mutate()}><Link2 /> ยอดตรงแล้ว - สร้างรายการรวมเพื่อตรวจ</button></div>
  </main>;
}
...
function MorePage() {
  ...
  return <main><PageHeader title="เพิ่มเติม" subtitle="สถานะระบบและข้อมูลผู้ส่ง" />
    <section className="info-list"><div><Bot /><span><strong>AI {ai.data?.enabled ? 'เปิด' : 'ปิด'}</strong><small>รอ {ai.data?.pending_count ?? ai.data?.queued ?? 0} · {ai.data?.model || '-'}</small></span></div><div><Users /><span><strong>ผู้ส่งที่รู้จัก</strong><small>{senders.data?.length || 0} คน</small></span></div>
      <Link to="/agents"><Bot /><span><strong>Agent Health</strong><small>ดู Shadow AI และประวัติการตัดสินใจ</small></span><ChevronRight /></Link>
...
function AgentHealthPage() {
  ...
  return <main><PageHeader title="Agent Health" subtitle="Shadow AI สังเกตการณ์ ไม่เปลี่ยนข้อมูลแทนคน" back />
    <ErrorBox error={health.error || decisions.error} />
    <section className="agent-health-grid">
      <div><span>โหมด</span><strong>{health.data?.shadow_mode ? 'Shadow' : '-'}</strong></div>
    ...
    <div className="agent-safety"><Check size={18} /><span><strong>ธุรกรรมยังทำต่อได้เมื่อ AI ล่ม</strong><small>AI เขียนได้เฉพาะผลทำนายและ audit log</small></span></div>
    <section className="section-block"><div className="section-title"><h2>การตัดสินใจล่าสุด</h2><span>{decisions.data?.length || 0}</span></div>
      <div className="agent-run-list">{(decisions.data || []).map((row) => { const state = row.status === 'cancelled' ? 'cancelled' : (row.comparison_status || row.shadow_status || row.status); return <article key={row.id}>
        <div><strong>{row.action_key}</strong><small>{new Date(row.created_at).toLocaleString('th-TH')} · {row.reason_code || 'รอเหตุผล'}{row.evidence?.length ? ` · หลักฐาน ${row.evidence.length}` : ''}</small></div>
        <b className={state}>{state === 'cancelled' ? 'ยกเลิกก่อนทำ' : state}</b>
