...
const bucketInfo: Record<Bucket, { label: string; tone: string }> = {
  ai_pending: { label: 'รอ AI อ่านรูป', tone: 'blue' },
  review: { label: 'รอคุณตรวจ', tone: 'amber' },
  needs_amount: { label: 'ยอดเงินมีปัญหา', tone: 'red' },
  bill: { label: 'บิลยังไม่มีสลิป', tone: 'blue' },
  slip: { label: 'สลิปยังไม่มีบิล', tone: 'purple' },
...
function BottomNav() {
  const links = [
    ['/', Home, 'หน้าหลัก'], ['/calendar', CalendarDays, 'เลือกวัน'], ['/search', Search, 'ค้นหา'],
    ['/more', Menu, 'เมนูอื่น']
  ] as const;
...
function WorkHome() {
  ...
  return <main>
    ...
    <label className="simple-filter"><span>ตอนนี้กำลังดู</span><select value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value)}><option value="">ทุกกลุ่ม LINE</option>{groupOptions.map((row) => <option key={row.source_id} value={row.source_id}>{groupName(row.source_id, groups.data)}</option>)}</select></label>
    {next && <section className="focus-card">
      <div className="focus-label"><span>ระบบแนะนำให้ทำรายการนี้ก่อน</span><b>{next.business_date === today ? 'ของวันนี้' : 'งานค้างล่าสุด'}</b></div>
      ...
      <Link className="primary-button" to={`/day/${next.business_date}/${next.source_id}?bucket=${nextBucket}`}><Eye /> เริ่มทำรายการนี้</Link>
    </section>}
    ...
    <section className="task-shortcuts" aria-label="เลือกงานอื่น"><h2>หรือเลือกงานอื่น</h2>
    ...
    <section className="section-block">
      <div className="section-title"><h2>รอบงานอื่นที่ยังค้าง</h2><Link to="/calendar">เลือกจากปฏิทิน</Link></div>
...
function DayGroupSheet({ date, rows, groups, onClose }: { date: string; rows: Row[]; groups: Row[]; onClose: () => void }) {
  return <div className="sheet-backdrop"><section className="choice-sheet"><header><div><h2>{shortDate(date)}</h2><p>เลือกกลุ่มที่ต้องการเปิด</p></div><button className="icon-button" onClick={onClose} aria-label="ปิด"><X /></button></header>
...
function ReviewPage() {
  ...
  return <main className="review-page">
    ...
    {match && !isConfirmed && <div className="review-steps" aria-label="ขั้นตอนตรวจ">
      ...
      <div><b>3</b><span>กดยืนยัน</span></div>
    </div>}
    {match && <section className={`decision-banner ${exact ? 'exact' : 'mismatch'}`}>
      {exact ? <BadgeCheck /> : <AlertTriangle />}<div><strong>{exact ? 'ยอดบิลกับยอดโอนตรงกัน' : `ยอดไม่ตรงกัน ต่าง ${money(Math.abs(difference))} บาท`}</strong><span>{exact ? 'ดูชื่อร้านและรูปทั้งสองให้ถูกก่อนกดยืนยัน' : 'กดแก้ยอดบิล หรือเลือกเอกสารคู่ใหม่'}</span></div>
    </section>}
    ...
    {bill && slip && <div className="evidence-switch" role="tablist" aria-label="เลือกหลักฐาน">
      <button className={activeEvidence?.id === bill.id ? 'active' : ''} onClick={() => setActiveEvidenceId(Number(bill.id))}><span>1. ดูบิล</span><strong>{money(itemAmount(bill))} บาท</strong></button>
      <button className={activeEvidence?.id === slip.id ? 'active' : ''} onClick={() => setActiveEvidenceId(Number(slip.id))}><span>2. ดูสลิป</span><strong>{money(itemAmount(slip))} บาท</strong></button>
    </div>}
    ...
    <div className="context-actions">
      <button className="context-button" onClick={() => setContextMode('chat')}><MessageCircle /><span><strong>ดูข้อความ LINE รอบรูปนี้</strong><small>ใช้เมื่อต้องดูว่าใครส่งและพิมพ์ว่าอะไร</small></span><ChevronRight /></button>
      {match && <button className="context-button" onClick={() => setContextMode('ai')}><Bot /><span><strong>ดูว่า AI เลือกคู่นี้เพราะอะไร</strong><small>เปิดดูเหตุผลและความมั่นใจ</small></span><ChevronRight /></button>}
    </div>
...
function ImageSheet({ item, bill, slip, onClose, onChange }: { item: Row; bill?: Row | null; slip?: Row | null; onClose: () => void; onChange: (item: Row) => void }) {
  return <div className="sheet-backdrop"><section className="full-sheet image-sheet"><header><div><h2>{item.id === bill?.id ? 'บิล' : item.id === slip?.id ? 'สลิป' : 'เอกสาร'}</h2><p>{money(itemAmount(item))} บาท · รูป #{item.id}</p></div><button className="icon-button" onClick={onClose} aria-label="ปิด"><X /></button></header>
...
function BatchSplit({ item, onClose }: { item: Row; onClose: () => void }) {
  ...
  const enteredLines = lines.filter((line) => line.supplier_name.trim() || String(line.amount).trim());
  const canSave = enteredLines.length > 0 && enteredLines.every((line) => {
