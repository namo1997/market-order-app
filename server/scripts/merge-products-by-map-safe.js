...
const readStringArg = (prefix, fallback = '') => {
  const found = args.find((arg) => arg.startsWith(`${prefix}=`));
  if (!found) return fallback;
  return found.split('=').slice(1).join('=').trim();
};

const pairsArg = readStringArg('--pairs', '');
...
const log = (...values) => console.log('[merge-products-by-map]', ...values);

const parsePairs = () => {
  if (!pairsArg) {
    throw new Error('ต้องระบุ --pairs=old:new,old:new');
  }

  const chunks = pairsArg
    .split(',')
    .map((v) => v.trim())
    .filter(Boolean);

  if (chunks.length === 0) {
    throw new Error('รูปแบบ --pairs ไม่ถูกต้อง');
  }
  ...
  chunks.forEach((chunk) => {
    const [oldRaw, newRaw] = chunk.split(':').map((v) => String(v || '').trim());
    const oldProductId = Number(oldRaw);
    const newProductId = Number(newRaw);

    if (!Number.isInteger(oldProductId) || !Number.isInteger(newProductId)) {
      throw new Error(`คู่ไม่ถูกต้อง: ${chunk}`);
    }
    if (oldProductId <= 0 || newProductId <= 0 || oldProductId === newProductId) {
      throw new Error(`คู่ไม่ถูกต้อง: ${chunk}`);
    }
    if (seenOldIds.has(oldProductId)) {
      throw new Error(`old id ซ้ำ: ${oldProductId}`);
    }
    seenOldIds.add(oldProductId);
    parsed.push({ oldProductId, newProductId });
  });
...
const getMappings = async () => {
  ...
  const idSet = new Set();
  pairs.forEach((p) => {
    idSet.add(p.oldProductId);
    idSet.add(p.newProductId);
  });

  const idList = [...idSet];
  const placeholders = idList.map(() => '?').join(', ');
  const [rows] = await connection.query(
    `SELECT id, name, is_active FROM products WHERE id IN (${placeholders})`,
    idList
  );
  const byId = new Map(rows.map((r) => [Number(r.id), r]));

  const [storeRows] = await connection.query(
    `SELECT DISTINCT pgl.product_id
     FROM product_group_links pgl
     JOIN product_groups pg ON pg.id = pgl.product_group_id
     WHERE pgl.product_id IN (${placeholders})
       AND pg.name IN (?, ?)`,
    [...idList, ...STORE_GROUP_NAMES]
  );
  const storeProductIds = new Set(storeRows.map((row) => Number(row.product_id)));

  return pairs.map((pair) => {
    const oldRow = byId.get(pair.oldProductId);
    const newRow = byId.get(pair.newProductId);

    if (!oldRow) throw new Error(`ไม่พบสินค้า old id=${pair.oldProductId}`);
    if (!newRow) throw new Error(`ไม่พบสินค้า new id=${pair.newProductId}`);
    if (Number(oldRow.is_active) !== 1) throw new Error(`old id=${pair.oldProductId} ไม่ active`);
    if (Number(newRow.is_active) !== 1) throw new Error(`new id=${pair.newProductId} ไม่ active`);
    if (!storeProductIds.has(pair.oldProductId)) {
      throw new Error(`old id=${pair.oldProductId} ไม่ได้อยู่ในกลุ่มสโตร์`);
    }
    if (!storeProductIds.has(pair.newProductId)) {
      throw new Error(`new id=${pair.newProductId} ไม่ได้อยู่ในกลุ่มสโตร์`);
    }

    return {
      oldProductId: pair.oldProductId,
      newProductId: pair.newProductId,
      label: `${oldRow.name} -> ${newRow.name}`
    };
  });
};

const createMergeMapTempTable = async (mappings) => {
  ...
  mappings.forEach((mapping) => {
    keepIds.add(mapping.newProductId);
    values.push([mapping.oldProductId, mapping.newProductId, mapping.label]);
    scopeValues.push([mapping.oldProductId, mapping.newProductId]);
  });
...
const getMappingStats = async () => {
  const [[summary]] = await connection.query(
    `SELECT
       ...
       COUNT(DISTINCT old_product_id) AS mapping_pairs
     FROM tmp_merge_map`
  );
...
const run = async () => {
  try {
    ...
    if (mappings.length === 0) {
      log('ไม่พบ mapping ที่ต้องรวม');
      return;
    }

    await createMergeMapTempTable(mappings);
    const { summary, sample } = await getMappingStats();
    log('mapping_pairs=', summary.mapping_pairs, 'merge_rows=', summary.merge_rows);
    log('sample (first 20):');
    sample.forEach((row) => {
...
run().catch((error) => {
  console.error('[merge-products-by-map] failed:', error.message);
  if (error?.details) {
