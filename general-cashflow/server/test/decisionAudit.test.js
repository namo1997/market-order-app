...
import { sanitizeAuditValue } from '../src/agents/decisionAudit.js';
...
test('decision audit redacts PINs and other credentials recursively', () => {
  assert.deepEqual(
    sanitizeAuditValue({ pin: '197019', nested: { password: 'secret' }, rows: [{ token: 'abc' }], label: 'cashier' }),
    { pin: '[REDACTED]', nested: { password: '[REDACTED]' }, rows: [{ token: '[REDACTED]' }], label: 'cashier' }
  );
});
